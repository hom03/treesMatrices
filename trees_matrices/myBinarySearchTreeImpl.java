...
public class myBinarySearchTreeImpl<T1 extends Comparable<? super T1>, T2> implements myBinarySearchTree<T1, T2> {
	...
	//-------------------------------------------------------------------
	// 17. ASSIGNMENT 1 - TASK 1A --> Count how many nodes are there at level k of myBinarySearchTree: my_count_at_level
	//-------------------------------------------------------------------
	public int my_count_at_level(int level) {
		int count = count_nodes_at_level(root,level);
		if (count == 0){
			System.out.println("There are no nodes at level "+ level);
		}
		return count;
		/*
		the worst-case runtime scenario is O(n) where n is the level symbolising how deep down the tree we are. Its really
		level dependant but could result in every node being visited and taking a long time.
		 */
	}
	private int count_nodes_at_level(myBinarySearchNode<T1,T2> current,int level){
		...
		if (current.getLeft() != null){
			leftCounter = count_nodes_at_level(current.getLeft(),level -1);
		}
		if (current.getRight() != null) {
			rightCounter = count_nodes_at_level(current.getRight(), level - 1);
		}
		return leftCounter + rightCounter;
	}

	//-------------------------------------------------------------------
	// 18. ASSIGNMENT 1 - TASK 1B --> Check if myBinarySearchTree is balanced: my_is_balanced
	//-------------------------------------------------------------------
	public boolean my_is_balanced() {
		int leftCounter = my_left_tree().my_node_count();
		int rightCounter = my_right_tree().my_node_count();
		if (leftCounter == rightCounter){
			System.out.println("This Binary Search tree is balanced.");
			return true;
			/*
			balanced tree asymptotic worst case runtime is O(log n) because the height of the tree would be
			logarithmic due to the number of nodes.
			 */

		}else{
			System.out.println("This Binary Search tree is not balanced.");
			return false;
			/*
			unbalanced tree asymptotic worst case runtime is O(n) because it has to visit every node
			 */
		}
	}

	//-------------------------------------------------------------------
	// 19. ASSIGNMENT 1 - TASK 1C --> Count how many nodes are smaller in myBinarySearchTree: my_count_smaller_nodes
	//-------------------------------------------------------------------
	public int my_count_smaller_nodes(T1 key) {
		return small_node_counting(root,key);
	}

	private int small_node_counting(myBinarySearchNode<T1,T2> current,T1 key){
		int smaller_count = 0;
		if (current == null){
			return 0;
		}
		int resCompare= key.compareTo(current.getKey());
		if (resCompare >0){
			smaller_count++;
		}
		smaller_count += small_node_counting(current.getLeft(),key);
		smaller_count += small_node_counting(current.getRight(),key);
		return smaller_count;
		/*
		the asymptotic worst case run time for this would be O(n) because the method would need to visit all nodes.
		 */
	}

	//-------------------------------------------------------------------
	// 20. ASSIGNMENT 1 - TASK 1D --> Return the level in myBinarySearchTree were the node is located : my_find_node_at_level
	//-------------------------------------------------------------------
	public int my_find_node_at_level(T1 key) {
		return findNodeLevel(root,key,0);
	}
	private int findNodeLevel(myBinarySearchNode<T1,T2> current ,T1 key, int level){
		if (current == null){
			return -1; //key wasnt found
		}
		int resCompare= key.compareTo(current.getKey());
		if (resCompare == 0){
			return level;// node found so return the level
		}else if (resCompare < 0){
			return findNodeLevel(current.getLeft(),key,level+1);// search left subtree
		}else {
			return findNodeLevel(current.getRight(), key, level + 1);// search right subtree for node
		}
		/*
		the asymptotic worst case event is when the tree is unbalanced and the time complexity is O(n) where n is the number of nodes
		 */
